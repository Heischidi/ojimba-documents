  ...
  return (
    <div className="max-w-2xl">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="font-display font-bold text-2xl text-gray-900">Add Product</h1>
        ...
        <div className="card-elevated p-6 rounded-2xl">
          <form onSubmit={handleSubmit(onSubmitDetails)} className="space-y-5">
            <div>
              ...
              <input type="text" placeholder="AI Engineering Course Notes" className={`input ${errors.name ? "border-red-400" : ""}`} {...register("name")} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  step="1"
                  placeholder="15000"
                  className={`input ${errors.price_naira ? "border-red-400" : ""}`}
                  {...register("price_naira", { valueAsNumber: true })}
                />
                {errors.price_naira && <p className="mt-1 text-sm text-red-600">{errors.price_naira.message}</p>}
            ...
            <button type="submit" disabled={saving} className="btn-primary w-full py-3.5 justify-center">
              {saving ? <><Loader2 className="w-4 h-4 animate-spin" /> Creating…</> : "Create Product →"}
            </button>
          </form>
        </div>
      )}
      ...
      {step === "files" && (
        <div className="space-y-4">
          {/* Digital file upload */}
          <div className={`card p-6 rounded-2xl border-2 ${fileUploaded ? "border-emerald-300 bg-emerald-50" : "border-dashed border-gray-300"}`}>
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${fileUploaded ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mb-3">PDF, DOCX, ZIP, MP4, XLSX, etc. Max {process.env.NEXT_PUBLIC_MAX_FILE_SIZE_MB || 500}MB</p>
          ...
          {/* Thumbnail upload */}
          <div className={`card p-6 rounded-2xl border-2 ${thumbUploaded ? "border-emerald-300 bg-emerald-50" : "border-dashed border-gray-200"}`}>
            <div className="flex items-start gap-4">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${thumbUploaded ? "bg-emerald-100 text-emerald-600" : "bg-gray-100 text-gray-500"}`}>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={handlePublish} className="btn-primary flex-1 py-3.5 justify-center">
              Publish Product
            </button>
            <button onClick={handleSaveDraft} className="btn-secondary px-6 py-3.5">
              Save as Draft
            </button>
          </div>
        </div>
